		...
		if (configure.subtype == "static") {
			...
			let config = [{
				...
				resolve: {
					...
					fallback: {
				      // These fallbacks are often necessary when using AWS SDK v3 in Webpack
				      fs: false,
				      net: false,
				      tls: false,
				      url: false,
							...
							options: {
								...
								presets: [
									...
									"@babel/preset-react",
									{
										...
										// "importSource": "react", // defaults to react (only in automatic runtime)
										"debug": false
									}
									],             
									["@babel/preset-env", {
								...
								plugins: [
									...
									'@babel/plugin-syntax-jsx',
										[
											...
											"regenerator": false // Enables async/await transformations  
											}
										]
								]								
							}
...
async function buildLambdaDirectory(rootDir, opts) {
	...
	let b = browserify({
		...
		insertGlobalVars: {
		  process: function () {
		    return 'global.process = global.process || { env: ' + JSON.stringify(process.env) + ' }';
		  },
		  _process: function () {
		    return 'process';
		  }
	...
	fs.readdirSync(path.resolve(__dirname, "../node_modules/@aws-sdk")).forEach((moduleName) => {
		b.external(`@aws-sdk/${moduleName}`);
	});
	
	// const requiredSmithyModules = [
	//   'config-resolver',
	//   'core',
	...
	// // Exclude all `@smithy` modules except the required ones
	// fs.readdirSync(path.resolve(__dirname, "../node_modules/@smithy")).forEach((moduleName) => {
	//   if (!requiredSmithyModules.includes(moduleName)) {
	//     b.external(`@smithy/${moduleName}`);
	//   }
	// });
	...
	try {
		...
		babelPlugins = []; // Blow away set presets otherwise .babelrc presets will not be used
	} catch (e) {
	...
	b.transform(babelify, {
		...
		exclude: ["node_modules", "/node_modules\/(?!@aws-sdk|@smithy|tedious)"],
		...
		if (build && build.include) {
			for (let i = 0; i < build.include.length; i++) {
				...
				if (!fs.existsSync(src)) {
					...
					if (!found) {
						throw new Error(`Unable to find source file ${origSrc} ${src}`);
					}
				}
