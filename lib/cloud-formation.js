...
module.exports = {
	createCloudFormation: async function (dir, opts) {
		...
		try {
			...
			console.log("path: ", path.resolve(dir, "*(bots|api)/{,!(node_modules)/**/}" + `/${opts.filter}/package.json`));
			...
			const entries = [];
			let variationFiles = []
			var processFiles = files.filter(f => !f.match(/\/node_modules\//)).map(f => {
				const pkg = merge({
					config: {
						leo: {}
					}
			...
			processFiles = [...processFiles, ...variationFiles]
			...
			if (swaggerString !== prevSwaggerString) {
				...
				cloudFormation.Resources[dkey] = {
					...
					DependsOn: ["RestApi", ...Object.keys(seenLambdaResourceNames).filter(key => !/Part[0-9]/.test(key))],
					Properties: {
						RestApiId: {
							Ref: "RestApi"
						},
				...
				// var deploymentId = dkey
				// dkey = "ApiStage" + Date.now();
				// cloudFormation.Resources[dkey] = {
	...
	buildAndPublish: async function (dir, buildDir, entries, cloudFormation, opts, microservice, buckets, hasApp, version) {
			...
			try {
				...
				// console.log("[calling build]", opts,dir,params)
				let data = await cmds.build(opts, dir, {
					buildDir: buildDir,
					lambdas: entries,
					cloudFormation: cloudFormation,
					alias: opts.alias,
				...
				if (fs.existsSync(publishjs)) {
					tasks.push(async () => { 
						await require(publishjs)(buildDir, cloudFormation); 
					});
				}

				const deployedStatic = {};
				if (true || opts.publish !== false) {
					// console.log("[opts]", opts, buckets)
					buckets.forEach(async (bucket) => {
						...
						const profile = bucket?.target?.leoaws?.config?.profile || bucket?.target?.leoaws?.config?.credentials || null;
				...
				console.error('Error in Build And Publish:',err)
				throw err
			}
	}
};
