	...
	createCloudFormation: function(dir, opts) {
		return new Promise((resolve, reject) => {
			...
			this.getBuckets(opts.targets, {
				...
				globSync(path.resolve(path.resolve(dir, "*(bots|api)/{,!(node_modules)/**/}" + `/${opts.filter}/package.json`)), {
					nodir: true
				}, function(err, files) {
					var entries = [];
					let processFiles = [];
					...
					globSync(path.resolve(path.resolve(dir, "*cloudformation/{,!(node_modules)/**/}" + `/*.js`)), {
						nodir: true
					}, function(err, files) {
						files.forEach(file => {
