...
module.exports = {
  createCloudFormation: async function (dir, opts) {
    ...
    try {
      ...
      const files = await new Promise((resolve, reject) => {
        glob(path.resolve(dir, "*(bots|api)/{,!(node_modules)/**/}" + `/${opts.filter}/package.json`), { nodir: true }, (err, files) => {
      ...
      if (swaggerString !== prevSwaggerString) {
        ...
        const dkey = "ApiDeployment" + Date.now();
  ...
  overrideCloudFormation: async function (dir, cloudFormation) {
    const files = await new Promise((resolve, reject) => {
      glob(path.resolve(dir, "*cloudformation/{,!(node_modules)/**/}" + `/*.js`), { nodir: true }, (err, files) => {
        if (err) return reject(err);
        resolve(files);
      });
    });
            ...
            if (opts.publish !== false) {
              if (!opts.cloudFormationOnly) {
                tasks.push(async done => {
                  ...
                  await cmds.publish(buildDir, `s3://${bucket.bucket}/${microservice.name}${opts.tag}/${version}/`, {
                    public: isPublic,
                    profile: profile,
                ...
                tasks.push(async done => {
                  await cmds.publish(path.resolve(buildDir, "cloudformation.json"), `s3://${bucket.bucket}/${microservice.name}${opts.tag}/${version}/cloudformation.json`, {
                    public: isPublic,
                    command: "cp",
